...
const SubMarketors: React.FC = () => {
      ...
      investors.forEach(inv => {
          inv.investments.forEach(investment => {
              if (investment.subMarketorId === subId) {
                  ...
                  // Find parent portfolio for this investment
                  let parentPortfolioName = 'Direct';
                  let marketorCommission = '0';
                  
                  // Look for the portfolio that contains this sub-marketor
                  portfolios.forEach(portfolio => {
                      const subMarketor = portfolio.subMarketors?.find(sm => sm.id === subId);
                      if (subMarketor) {
                          parentPortfolioName = portfolio.name;
                          marketorCommission = String(portfolio.defaultCommissionRate || '0');
                      }
                  });
                  ...
                  associatedInvestments.push({
                      ...
                      marketorCommission: marketorCommission,
                      subMarketorCommission: investment.subMarketorCommission || '0',
                      portfolioName: parentPortfolioName,
                      id: investment.id
                  });
              }
          });
  ...
  const renderSubMarketorDashboard = (sub: SubMarketor, parent: Portfolio) => {
      ...
      return (
        <div className="space-y-6 animate-fade-in-up">
                ...
                <div className="lg:col-span-3">
                    <Card>
                       <div className="flex flex-col items-center pb-6 border-b border-gray-100">
                           ...
                           <span className="text-xs text-indigo-600 font-bold mt-2">Portfolio: {parent.name}</span>
                       </div>
           ...
           {/* Investments List - Updated to match Portfolios page concept */}
                ...
                <Table<any>
                   columns={[
                       ...
                       },
                       {
                           ...
                           render: (item) => (
                               <div>
                                   <div className="text-xs text-gray-700 font-medium">{sub.name}</div>
  ...
  if (selectedSubMarketor) {
      return (
          <>
              {renderSubMarketorDashboard(selectedSubMarketor.sub, selectedSubMarketor.parent)}
                  ...
                  <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
                      <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl p-6 animate-fade-in-up">
                           <div className="flex justify-between items-center mb-6">
                               <h3 className="text-xl font-bold text-navy-700">{isEditing ? 'Edit Sub-Marketer' : 'Add New Sub-Marketer'}</h3>
                          ...
                          <form onSubmit={handleSubmit}>
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                  <div>
                                      <label className="block text-xs font-bold text-gray-500 mb-1">Portfolio/Marketer *</label>
                                      <select
                                          name="portfolioId"
                                  ...
                                  <div><label className="block text-xs font-bold text-gray-500 mb-1">Name *</label><input name="name" value={formData.name} onChange={handleInputChange} required className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm" /></div>
                                  <div><label className="block text-xs font-bold text-gray-500 mb-1">Email *</label><input name="email" value={formData.email} onChange={handleInputChange} required className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm" /></div>
                                  <div><label className="block text-xs font-bold text-gray-500 mb-1">Phone *</label><input name="phone" value={formData.phone} onChange={handleInputChange} required className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm" /></div>
                                  <div><label className="block text-xs font-bold text-gray-500 mb-1">Commission Rate (%)</label><input name="commissionRate" type="number" step="0.01" value={formData.commissionRate} onChange={handleInputChange} className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm" /></div>
                                  <div><label className="block text-xs font-bold text-gray-500 mb-1">PAN</label><input name="pan" value={formData.pan} onChange={handleInputChange} className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm" /></div>
                                  <div><label className="block text-xs font-bold text-gray-500 mb-1">Aadhar</label><input name="aadhar" value={formData.aadhar} onChange={handleInputChange} className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm" /></div>
                              </div>

                              <h5 className="text-sm font-bold text-gray-700 uppercase mb-4 border-b pb-1">Address Details</h5>
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                  <div className="md:col-span-2"><label className="block text-xs font-bold text-gray-500 mb-1">Address</label><input name="address" value={formData.address} onChange={handleInputChange} className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm" /></div>
                                  <div><label className="block text-xs font-bold text-gray-500 mb-1">City</label><input name="city" value={formData.city} onChange={handleInputChange} className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm" /></div>
                                  <div><label className="block text-xs font-bold text-gray-500 mb-1">State</label><input name="state" value={formData.state} onChange={handleInputChange} className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm" /></div>
                                  <div><label className="block text-xs font-bold text-gray-500 mb-1">Pincode</label><input name="pincode" value={formData.pincode} onChange={handleInputChange} className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm" /></div>
                              </div>
                              ...
                              <div className="mb-4">
                                  ...
                                  {formData.photoFile && (
                                      <img
                                          src={formData.photoFile instanceof File
                              ...
                              <h5 className="text-sm font-bold text-gray-700 uppercase mb-4 border-b pb-1 flex justify-between">
                                  Bank Details
                                  <button type="button" onClick={addBank} className="text-xs bg-indigo-50 text-indigo-600 px-2 py-1 rounded hover:bg-indigo-100">+ Add Bank</button>
                              </h5>
                              {formData.bankAccounts.map((bank, index) => (
                                  <div key={bank.id} className="bg-gray-50 p-3 rounded-lg mb-3 relative border border-gray-200">
                                      ...
                                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                          <div><label className="text-[10px] font-bold text-gray-500">IFSC</label><input value={bank.ifsc} onChange={(e) => handleBankChange(index, 'ifsc', e.target.value)} onBlur={() => handleBankIfscBlur(index)} className="w-full rounded border px-2 py-1 text-xs" /></div>
                                          <div><label className="text-[10px] font-bold text-gray-500">Bank Name</label><input value={bank.bankName} onChange={(e) => handleBankChange(index, 'bankName', e.target.value)} className="w-full rounded border px-2 py-1 text-xs" /></div>
                                          <div><label className="text-[10px] font-bold text-gray-500">Account No</label><input value={bank.accountNumber} onChange={(e) => handleBankChange(index, 'accountNumber', e.target.value)} className="w-full rounded border px-2 py-1 text-xs" /></div>
                                          <div><label className="text-[10px] font-bold text-gray-500">Holder Name</label><input value={bank.accountHolderName} onChange={(e) => handleBankChange(index, 'accountHolderName', e.target.value)} className="w-full rounded border px-2 py-1 text-xs" /></div>
                                      </div>
                                  </div>
                              ))}

                              <div className="flex justify-end gap-3 mt-6">
                                  <button type="button" onClick={handleCancel} className="px-4 py-2 text-sm text-gray-600 hover:bg-gray-100 rounded-lg">Cancel</button>
  ...
  return (
    <>
      ...
      {/* Sub-Marketer Form Modal - Same as Portfolios page */}
      {showForm && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
              <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl p-6 animate-fade-in-up">
                   <div className="flex justify-between items-center mb-6">
